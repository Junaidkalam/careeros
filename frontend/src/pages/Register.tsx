...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const data = await fetchApi<{ token: string; email: string; name: string }>('/auth/register', {
        method: 'POST',
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-background">
      <div className="w-full max-w-[380px]">
        <div className="text-center mb-10">
          ...
          <p className="text-[13px] text-muted-foreground mt-1">Create your account</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input type="text" required value={name} onChange={(e) => setName(e.target.value)} className="bg-card text-foreground" />
          </div>
          <div className="space-y-1.5">
            <label className="text-[13px] font-medium text-muted-foreground">Email</label>
            <Input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="bg-card text-foreground" />
          </div>
          <div className="space-y-1.5">
            <label className="text-[13px] font-medium text-muted-foreground">Password</label>
            <Input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="bg-card text-foreground" />
          </div>
